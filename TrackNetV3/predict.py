...
def expand_sparse_predictions(pred_dict, video_len):
    full = {
        'Frame': list(range(video_len)),
        'X': [0] * video_len,
        'Y': [0] * video_len,
        'Visibility': [0] * video_len,
    }
    for frame, x, y, visibility in zip(
        pred_dict['Frame'], pred_dict['X'], pred_dict['Y'], pred_dict['Visibility']
    ):
        if 0 <= frame < video_len:
            full['X'][frame] = x
            full['Y'][frame] = y
            full['Visibility'][frame] = visibility
    return full
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ranges_file', type=str, default='', help='JSON file containing sparse TrackNet intervals')
    ...
    frame_ranges = None
    if args.ranges_file:
        if args.eval_mode != 'nonoverlap' or not large_video:
            parser.error('--ranges_file requires --large_video and --eval_mode nonoverlap')
        with open(args.ranges_file, 'r', encoding='utf-8') as file:
            range_payload = json.load(file)
        frame_ranges = range_payload.get('intervals', range_payload)
    ...
    # Load model
    tracknet_ckpt = torch.load(args.tracknet_file, map_location='cuda', weights_only=False)
    ...
    if args.inpaintnet_file:
        inpaintnet_ckpt = torch.load(args.inpaintnet_file, map_location='cuda', weights_only=False)
    ...
    range_alignment = tracknet_seq_len
    if inpaintnet is not None:
        range_alignment = tracknet_seq_len * inpaintnet_seq_len // math.gcd(tracknet_seq_len, inpaintnet_seq_len)
    ...
    if args.eval_mode == 'nonoverlap':
        ...
        if large_video:
            dataset = Video_IterableDataset(video_file, seq_len=seq_len, sliding_step=seq_len, bg_mode=bg_mode, 
                                            ...
                                            frame_ranges=frame_ranges, range_alignment=range_alignment)
            ...
            if frame_ranges is not None:
                selected_frames = sum(end - start for start, end in dataset.frame_ranges)
                print(f'Sparse TrackNet frames: {selected_frames} ({selected_frames / dataset.video_len:.1%})')
        else:
    ...
    if frame_ranges is not None:
        pred_dict = expand_sparse_predictions(pred_dict, video_len)
