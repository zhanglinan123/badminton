...
class Shuttlecock_Trajectory_Dataset(Dataset):
    ...
    def _gen_input_from_pred_dict(self):
        ...
        frame_ids = self.pred_dict.get('Frame', list(range(len(x_pred))))
        ...
        if self.padding and self.sliding_step == self.seq_len:
            sample_count = len(inpaint)
            if sample_count == 0:
            ...
            padding = (-sample_count) % self.seq_len
            frame_array = np.asarray(frame_ids, dtype=np.int32)
            coor_array = np.column_stack((x_pred, y_pred)).astype(np.float32)
            vis_array = np.asarray(vis_pred, dtype=np.float32)
            mask_array = np.asarray(inpaint, dtype=np.float32)
            if padding:
                frame_array = np.pad(frame_array, (0, padding), mode='edge')
                coor_array = np.pad(coor_array, ((0, padding), (0, 0)), mode='edge')
                vis_array = np.pad(vis_array, (0, padding), mode='edge')
                mask_array = np.pad(mask_array, (0, padding), mode='edge')

            frame_array = frame_array.reshape(-1, self.seq_len)
            id = np.stack((np.zeros_like(frame_array), frame_array), axis=-1)
            coor_pred = coor_array.reshape(-1, self.seq_len, 2)
            pred_vis = vis_array.reshape(-1, self.seq_len)
            inpaint_mask = mask_array.reshape(-1, self.seq_len)
            return dict(id=id, coor_pred=coor_pred, pred_vis=pred_vis, inpaint_mask=inpaint_mask), \
                   dict(img_scaler=self.pred_dict['Img_scaler'], img_shape=self.pred_dict['Img_shape'])
        ...
        for i in range(0, len(inpaint), self.sliding_step):
            ...
            for f in range(self.seq_len):
                if i+f < len(inpaint):
                    tmp_idx.append((0, frame_ids[i+f]))
                    tmp_coor_pred.append((x_pred[i+f], y_pred[i+f]))
                    ...
                    # Padding the last sequence if imcompleted
                    if self.padding:
                        tmp_idx.append((0, frame_ids[last_idx]))
                        tmp_coor_pred.append((x_pred[last_idx], y_pred[last_idx]))
...
class Video_IterableDataset(IterableDataset):
    ...
    def __init__(self,
        ...
        self.sliding_step = sliding_step
        self.bg_mode = bg_mode
        self.resize_pool = ThreadPoolExecutor(max_workers=min(8, os.cpu_count() or 1)) \
            if self.bg_mode == 'concat' else None
        self.frame_ranges = self.__normalize_frame_ranges__(frame_ranges, range_alignment)
        if self.frame_ranges is not None and self.sliding_step != self.seq_len:
            raise ValueError('Sparse frame ranges require non-overlap sampling.')
        if self.bg_mode:
    ...
    def __iter__(self):
        ...
        if self.frame_ranges is not None:
            yield from self.__iter_frame_ranges__()
            self.cap.release()
            return
    ...
    def __normalize_frame_ranges__(self, frame_ranges, alignment):
        if frame_ranges is None:
            return None

        normalized = []
        alignment = max(1, int(alignment))
        for interval in frame_ranges:
            start_seconds = float(interval['start'])
            end_seconds = float(interval['end'])
            start = max(0, math.floor(start_seconds * self.fps / alignment) * alignment)
            end = min(self.video_len, math.ceil(end_seconds * self.fps / alignment) * alignment)
            if end <= start:
                continue
    ...
    def __iter_frame_ranges__(self):
        for range_start, range_end in self.frame_ranges:
            self.cap.set(cv2.CAP_PROP_POS_FRAMES, range_start)
            frame_id = range_start
            while frame_id < range_end:
                frame_list = []
                frame_ids = []
                while len(frame_list) < self.seq_len and frame_id < range_end:
                    success, frame = self.cap.read()
                    if not success:
                        break
                    frame_list.append(frame)
                ...
                if len(frame_list) < self.seq_len:
                    padding = self.seq_len - len(frame_list)
                    frame_list.extend([frame_list[-1]] * padding)
                    frame_ids.extend([frame_ids[-1]] * padding)
                frames = self.__process__(np.asarray(frame_list)[..., ::-1])
                yield np.asarray([(0, current) for current in frame_ids]), frames
    ...
    def __process__(self, imgs):
        ...
        if self.bg_mode == 'concat':
            resized = np.asarray(list(self.resize_pool.map(self.__resize_frame__, imgs)), dtype=np.uint8)

            frames = np.empty(((self.seq_len + 1) * 3, self.HEIGHT, self.WIDTH), dtype=np.float32)
            frames[:3] = np.asarray(median_img, dtype=np.float64) / 255.
            frames[3:] = resized.transpose(0, 3, 1, 2).reshape(-1, self.HEIGHT, self.WIDTH) / 255.
            return frames
    ...
    def __resize_frame__(self, img):
        return np.asarray(
            Image.fromarray(img).resize(size=(self.WIDTH, self.HEIGHT)),
            dtype=np.uint8,
        )
