...
def generate_inpaint_mask(pred_dict, th_h=30):
    ...
    frame_ids = np.asarray(pred_dict.get('Frame', np.arange(len(y))))
    boundaries = np.flatnonzero(np.diff(frame_ids) != 1) + 1
    segment_starts = np.r_[0, boundaries]
    segment_ends = np.r_[boundaries, len(y)]
    ...
    for segment_start, segment_end in zip(segment_starts, segment_ends):
        ...
        segment_vis = vis_pred[segment_start:segment_end]
        segment_mask = np.zeros_like(segment_y)
        i = 0
        j = 0
        while j < len(segment_vis):
            while i < len(segment_vis)-1 and segment_vis[i] == 1:
                i += 1
            j = i
            while j < len(segment_vis)-1 and segment_vis[j] == 0:
                j += 1
            if j == i:
                break
            elif i == 0 and segment_y[j] > th_h:
                segment_mask[:j] = 1
            elif (i > 1 and segment_y[i-1] > th_h) and segment_y[j] > th_h:
                segment_mask[i:j] = 1
            i = j
        inpaint_mask[segment_start:segment_end] = segment_mask
